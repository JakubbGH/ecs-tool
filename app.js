...
async function loadDataFromCloud() {
    ...
    try {
        ...
        bSelect.innerHTML = "";
        database.sort((a, b) => a.building.localeCompare(b.building)).forEach(item => bSelect.add(new Option(item.building, item.building)));
...
function loadBuildingToTable() {
    ...
    if (match) {
        match.ecs_list.forEach(ecs => {
            ...
            row.innerHTML = `<td style="font-weight:bold">${bValue}</td><td>${ecs}</td><td><select style="width:100%;"><option>1HAND_POS</option><option>2PREP_ALMT</option><option>3WELDING</option><option>4PUNCH</option></select></td><td><button class="del-btn" onclick="this.parentElement.parentElement.remove()">DELETE</button></td>`;
        });
...
async function exportAllReports() {
    try {
        ...
        if (snap.empty) return alert("No reports available.");
